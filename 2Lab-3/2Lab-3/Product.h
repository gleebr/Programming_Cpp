...
private:
    std::string address;
    std::vector<std::string> forbiddenIngredients;
    int inspectionDate;

public:
    Commission(); // Конструкторы и деструктор
    Commission(const std::string& address_, const std::vector<std::string>& forbiddenIngredients_, int inspectionDate_);
    ...
    void SetForbiddenIngredients(const std::vector<std::string>& forbiddenIngredients_) { forbiddenIngredients = forbiddenIngredients_; }
    void SetInspectionDate(int inspectionDate_) { inspectionDate = inspectionDate_; }

    std::string getAddress() const { return address; }
    std::vector<std::string> getForbiddenIngredients() const { return forbiddenIngredients; }
    int getInspectionDate() const { return inspectionDate; }

    void Display() const; // Методы
    void addForbiddenIngredient(const std::string& ingredient);
