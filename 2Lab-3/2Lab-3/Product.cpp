...
void Ingredient::Display() const {
    std::cout << name << " (" << volume << " ед.)" << std::endl;
}

Commission::Commission() : address(""), inspectionDate(0) {} // Реализация Commission

Commission::Commission(const std::string& address_, const std::vector<std::string>& forbiddenIngredients_, int inspectionDate_)
...
void Commission::addForbiddenIngredient(const std::string& ingredient) { // Функция для добавления запрещённого ингредиента
    if (std::find(forbiddenIngredients.begin(), forbiddenIngredients.end(), ingredient) == forbiddenIngredients.end()) {
        forbiddenIngredients.push_back(ingredient);
        std::cout << "Ингредиент " << ingredient << " добавлен в список запрещённых" << std::endl;
    }
    else {
        std::cout << "Ингредиент " << ingredient << " уже есть в списке запрещённых" << std::endl;
    }
}
...
bool Commission::check(const Product& product) const {
    ...
    for (const auto& ingredient : composition) {
        if (std::find(forbiddenIngredients.begin(), forbiddenIngredients.end(), ingredient.name) != forbiddenIngredients.end()) {
            std::cout << "Товар не прошёл проверку: содержит запрещённый ингредиент " << ingredient.name << std::endl;
            return false;
        }
    }
...
bool Product::shouldRemove(const Ingredient& item, const std::vector<Ingredient>& toRemove) {
...
void Product::removeCommon_(std::vector<Ingredient>& target, const std::vector<Ingredient>& toRemove) {
    auto it = std::remove_if(target.begin(), target.end(),
        [&toRemove](const Ingredient& item) {
            return shouldRemove(item, toRemove);
        });
